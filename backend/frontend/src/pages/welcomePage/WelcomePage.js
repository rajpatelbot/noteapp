...
const WelcomePage = ({ darkMode, notes, setNotes }) => {
  ...
  const fetchNotes = async () => {
    try {
      let res = await axiosInstance.get("/api/notes", config);
      setNotes(res.data);
    } catch (err) {
  ...
  async function deleteNote(_id, user_id) {
    ...
    try {
      let res = await axiosInstance.delete(`api/notes/${user_id}`, deleteconfig);
      if (res.status === 201) {
        fetchNotes();
        setSuccessMsg(res.data);
        setTimeout(() => {
    ...
    <div className="note__create-form">
      ...
      <section className="noteapp__welcomePage-noteRender-parent-container padding">
        {notes.length === 0 ? (
          ...
          notes.map((note) => {
            if(note.title.trim().toLowerCase().includes(search) || note.description.trim().toLowerCase().includes(search)){
              return (
                <div key={note._id} className="noteapp__welcomePage-createdNote">
